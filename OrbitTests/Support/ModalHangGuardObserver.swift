...
final class ModalHangGuardObserver: NSObject, XCTestObservation {
    ...
    // Set by ModalHangGuardPrincipalClass.init() only.
    static var isInstalled = false

    // MARK: - Tuning

    // Longest currently legitimate modal lifetime is ~1.2-2s; raise this and say why here rather than working around it per-test.
    ...
    private func reportHangAndAbort(_ modalWindow: NSWindow) {
        ...
        Orbit/UI/Content/OrbitContextMenu.swift's buildNativeContextMenu vs \
        presentNativeContextMenu), or assert the pure decision function a delegate call would \
